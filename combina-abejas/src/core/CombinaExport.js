...
// ========== FUNCIÓN SPIN PARA MÓVIL (REESCRITA - SIN DESAPARECER) ==========
function spinMobile() {
    ...
    // Deshabilitar scroll durante la animación
    if (gameContainer) {
    ...
    // Posición donde está el avatar (arriba a la izquierda)
    ...
    // Variable para controlar la animación en curso
    let currentSlotIndex = 0;
    
    function processNextSlot() {
        if (currentSlotIndex >= slotViews.length) {
            // Terminaron todos los slots
            if (gameContainer) {
                gameContainer.style.overflowX = 'auto';
            }
            isSpinning = false;
        ...
        // Posiciones actuales (desde donde empezamos la animación)
        ...
        function animate() {
            ...
            if (isMovingToCenter) {
                ...
                if (animProgress >= 1) {
                    // Terminó movimiento al centro, comenzar giro
